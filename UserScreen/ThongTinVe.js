...
const handleSelectGhe = (vi_tri) => {
  setGheSelected((prev) => {
    const isSelected = prev.includes(vi_tri);

    // Nếu ghế đã được chọn -> bỏ chọn
    if (isSelected) {
      return prev.filter((g) => g !== vi_tri);
    }

    // 1. Giới hạn tối đa 8 ghế
    if (prev.length >= 8) {
      ToastAndroid.show("Bạn chỉ được đặt tối đa 8 ghế!", ToastAndroid.SHORT);
      return prev;
    }

    // 2. Kiểm tra không chọn xen kẽ ghế trong cùng hàng
    const row = vi_tri[0]; // Ví dụ "A1" => row = "A"
    const number = parseInt(vi_tri.slice(1), 10);

    // Lấy các ghế cùng hàng đã chọn
    const sameRow = prev
      .filter((g) => g[0] === row)
      .map((g) => parseInt(g.slice(1), 10));

    if (sameRow.length > 0) {
      const newRowSeats = [...sameRow, number].sort((a, b) => a - b);

      // Duyệt xem có ghế nào bị bỏ trống ở giữa không
      for (let i = 0; i < newRowSeats.length - 1; i++) {
        if (newRowSeats[i + 1] - newRowSeats[i] > 1) {
          ToastAndroid.show(
            "Không được chọn ghế xen kẽ, vui lòng chọn liền kề!",
            ToastAndroid.SHORT
          );
          return prev;
        }
